...
- 【接口文档】
  ...
  - 接口文档自动生成并更新，无需执行 `swag init`（开发时要配置 `project.home` 环境变量）
- 【对运维更加友好的系统配置】
  - 启动时，只需要在环境变量中设置 `ENV_xxx`（xxx 为对应的具体配置名），即可改变对应的配置参数。
  - 系统可内置三个环境配置（`prod`、`dev`、`local`），并可通过环境变量`RUN_ENV`指定启动时的环境配置。

- 【日志】
  - 使用zap 日志框架，高性能，对日志文件进行自动分割和控制日志文件占用的存储空间。
